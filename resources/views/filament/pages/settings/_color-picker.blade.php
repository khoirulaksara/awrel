<div x-data="awrelColorPicker('{{ $settings['primary_color'] ?? '#f59e0b' }}')" class="rounded-xl border border-gray-100 bg-gray-50/50 p-4 dark:border-gray-800 dark:bg-gray-900/50">
    ...
    <div class="mt-3 flex items-center gap-4">
        <input type="color" wire:model.live="settings.primary_color" x-ref="colorInput" x-on:input="updatePreview($refs.colorInput.value)" class="h-10 w-10 cursor-pointer rounded-lg border border-gray-300 dark:border-gray-600">
        <div class="flex-1">
            <input type="text" wire:model.live="settings.primary_color" class="block w-full rounded-lg border border-gray-300 bg-white px-3 py-2 text-sm text-gray-900 shadow-sm focus:border-primary-500 focus:ring-1 focus:ring-primary-500 dark:border-gray-600 dark:bg-gray-800 dark:text-gray-100" placeholder="#f59e0b" />
        </div>
